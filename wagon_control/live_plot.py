...
class LivePlotter:
    ...
    def __init__(self, run_dir: Path, update_interval: int = 500) -> None:
        ...
        # Create figure with subplots - dark mode (larger for more plots)
        self.fig = plt.figure(figsize=(18, 14), facecolor=MONUMENTAL_DARK_BLUE)
        self.fig.suptitle(
        ...
        # Row 1: GPS continues, Position L2 Error (col 1), Heading Error (col 2)
        # Row 2: Position Errors X/Y (col 0), (empty col 1), (empty col 2)
        self.ax_gps = plt.subplot2grid((3, 3), (0, 0), rowspan=2)
        self.ax_velocity = plt.subplot2grid((3, 3), (0, 1))
        self.ax_angular_velocity = plt.subplot2grid((3, 3), (0, 2))
        self.ax_position_l2 = plt.subplot2grid((3, 3), (1, 1))
        self.ax_heading_error = plt.subplot2grid((3, 3), (1, 2))
        self.ax_position_errors = plt.subplot2grid((3, 3), (2, 0))
        ...
        (self.gps_line,) = self.ax_gps.plot(
            [], [], "o-", color=MONUMENTAL_ORANGE, linewidth=1.0, alpha=0.6,
            markersize=4, label="GPS (1 Hz)"
        )
        ...
        (self.position_l2_line,) = self.ax_position_l2.plot(
            [], [], color=MONUMENTAL_ORANGE, linewidth=2, alpha=0.8, label="L2 Error"
        )
        (self.position_l2_mean,) = self.ax_position_l2.plot(
            [], [], "--", color=MONUMENTAL_BLUE, linewidth=1.5, alpha=0.7, label="Mean"
        )

        (self.heading_error_line,) = self.ax_heading_error.plot(
            [], [], color=MONUMENTAL_ORANGE, linewidth=1.5, alpha=0.8
        )
        ...
        (self.velocity_ref_line,) = self.ax_velocity.plot(
            [], [], color=MONUMENTAL_BLUE, linewidth=2, alpha=0.7, label="Reference"
        )
        (self.velocity_loc_line,) = self.ax_velocity.plot(
            [], [], color=MONUMENTAL_ORANGE, linewidth=1.5, alpha=0.8, label="Localized"
        )
    ...
    def _setup_axes(self) -> None:
        ...
        all_axes = [
            self.ax_gps, self.ax_velocity, self.ax_angular_velocity,
            self.ax_position_l2, self.ax_heading_error,
        ...
        legend = self.ax_gps.legend(
            loc="upper right", fontsize=8, facecolor=MONUMENTAL_DARK_BLUE,
            edgecolor=MONUMENTAL_CREAM, framealpha=0.9
        )
        ...
        self.ax_velocity.set_title("Linear Velocity (REF vs LOC)", fontsize=11, fontweight="bold")
        ...
        self.ax_angular_velocity.set_xlabel("Time (s)", fontsize=10)
        self.ax_angular_velocity.set_ylabel("Angular Velocity (rad/s)", fontsize=10)
        self.ax_angular_velocity.set_title("Angular Velocity (REF vs LOC)", fontsize=11, fontweight="bold")
        ...
        self.ax_position_l2.set_xlabel("Time (s)", fontsize=10)
        self.ax_position_l2.set_ylabel("L2 Error (m)", fontsize=10)
        self.ax_position_l2.set_title("Position L2 Error", fontsize=11, fontweight="bold")
        self.ax_position_l2.grid(True, alpha=0.2, color=MONUMENTAL_CREAM)
        legend = self.ax_position_l2.legend(
            loc="upper right", facecolor=MONUMENTAL_DARK_BLUE, edgecolor=MONUMENTAL_CREAM
        )
        plt.setp(legend.get_texts(), color=MONUMENTAL_CREAM)

        # Heading Error plot
        self.ax_heading_error.set_xlabel("Time (s)", fontsize=10)
        self.ax_heading_error.set_ylabel("Heading Error (°)", fontsize=10)
        self.ax_heading_error.set_title("Heading Error", fontsize=11, fontweight="bold")
        ...
        self.ax_position_errors.set_xlabel("Time (s)", fontsize=10)
        self.ax_position_errors.set_ylabel("Position Error (m)", fontsize=10)
        self.ax_position_errors.set_title("Position Errors (X/Y)", fontsize=11, fontweight="bold")
    ...
    def _load_state_data(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            ...
            with open(self.state_path, 'r') as f:
                ...
                data = {'timestamp': [], 'x_loc': [], 'y_loc': [], 'theta_loc': [], 'v_x': [], 'v_y': []}
                for row in reader:
                    try:
    ...
    def _load_reference_data(self) -> Optional[Dict[str, np.ndarray]]:
        ...
        try:
            ...
            with open(self.reference_path, 'r') as f:
                reader = csv.DictReader(f)
                data = {'timestamp': [], 'elapsed_time': [], 'x_ref': [], 'y_ref': [], 'theta_ref': []}
                for row in reader:
                    try:
                        data['timestamp'].append(float(row['timestamp']))
    ...
    def _update(self, frame: int) -> tuple:
        ...
        return (
            ...
            self.loc_line,
            self.traj_start,
            self.position_l2_line,
            self.position_l2_mean,
    ...
    def _update_angular_velocity(self) -> None:
        """Update angular velocity comparison plot showing ω_ref vs ω_loc."""
        try:
            # Load reference and IMU data
            ref_data = self._load_reference_data()
    ...
    def _update_diagnostic_plots(self) -> None:
        """Update all diagnostic plots with state and reference data."""
        try:
            # Load state and reference data
            state_data = self._load_state_data()
            ref_data = self._load_reference_data()
            ...
            error_theta = ref_data['theta_ref'] - state_data['theta_loc']
            ...
            # Update Position L2 Error plot
            self.position_l2_line.set_data(relative_times, error_l2)
            if len(error_l2) > 0:
                mean_error = np.mean(error_l2)
                self.position_l2_mean.set_data([relative_times[0], relative_times[-1]], [mean_error, mean_error])
            self.ax_position_l2.relim()
            self.ax_position_l2.autoscale_view()
            ...
            if len(error_theta_deg) > 0:
                ...
                self.heading_error_mean.set_data([relative_times[0], relative_times[-1]],
